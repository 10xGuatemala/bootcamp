...
Los cinco patrones anteriores resuelven problemas distintos y no se pisan entre sí. Conviene tener presente el mapa antes de abrir cualquiera de ellos en un servicio nuevo.

| Patrón | Úsalo cuando | Evítalo cuando |
|---|---|---|
| Strategy (filtrado) | Múltiples servicios con filtros similares; tentación de copiar `ApplyFilters` | Un solo servicio, con uno o dos filtros triviales |
| Factory | Creación compleja con validaciones, múltiples rutas que producen el mismo tipo | Creación directa con `new` sin reglas, objetos planos |
| Builder | Entidad con muchos campos, validación contextual, cálculos durante la construcción | DTOs simples, tres o cuatro propiedades, sin reglas cruzadas |
| Template Method | Varios servicios comparten flujo común (CRUD de catálogo, por ejemplo) | Servicios únicos sin lógica repetida |
| Máquina de estados | Entidad con tres o más estados y transiciones que no son todas válidas | Flag booleano o dos estados con transición libre |
...
**Pasos:**
1. Detectar duplicación en métodos de filtrado o paginación; si hay dos o más ocurrencias, aplicar Strategy.
2. Detectar métodos de creación con más de siete u ocho campos y validación cruzada; aplicar Builder.
3. Detectar varias rutas que crean el mismo tipo de entidad con reglas compartidas; aplicar Factory.
4. Detectar servicios de catálogo o CRUD similares repetidos; aplicar Template Method.
5. Detectar entidades con tres o más estados y transiciones restringidas; aplicar Máquina de Estados.
6. Si el sistema crecerá a varios módulos coordinados, priorizar Repository y arquitectura dirigida por eventos antes de agregar más patrones tácticos.
7. Revisar el mapa de decisión antes de aplicar cualquier patrón nuevo; evitar patrones donde una solución directa resuelve el caso.
